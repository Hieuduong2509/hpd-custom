...
const api = {
  ...
  useTeamApiKeys() {
    return useQuery<TeamApiKeysApiResponse>({
      queryKey: [`team/api-keys`],
      queryFn: () => hdxServer(`team/api-keys`).json<TeamApiKeysApiResponse>(),
    });
  },
  useCreateTeamApiKey() {
    return useMutation<
    ...
    >({
      mutationFn: async ({ name }) =>
        hdxServer(`team/api-keys`, {
          method: 'POST',
          json: { name },
        }).json<CreateTeamApiKeyApiResponse>(),
    });
  },
  useRevokeTeamApiKey() {
    return useMutation<
  ...
  useSetApiKeyMembers() {
    ...
    >({
      mutationFn: async ({ id, memberIds }) =>
        hdxServer(`team/api-keys/${id}/members`, {
          method: 'PUT',
          json: { memberIds },
        }).json<{ message: string }>(),
    });
  ...
  useUpdateTeamMemberRole() {
    return useMutation<
      { success: boolean },
      Error | HTTPError,
      { userId: string; role: 'admin' | 'owner' | 'dev' }
    >({
  ...
  useSaveTeamInvitation() {
    ...
    >({
      mutationFn: async ({ name, email, role, apiKeyId }) =>
