...
function DBServiceMapPage() {
  ...
  const serviceNameOptions = useMemo(() => {
    let services = (serviceNameValues?.[0]?.value ?? [])
      .map(v => String(v))
      .sort((a, b) => a.localeCompare(b));

    // UI-only convenience filter, NOT a security boundary: the ClickHouse
    // proxy (packages/api/src/routers/api/clickhouseProxy.ts) does not enforce
    // allowedServices server-side, so a dev can still query another service's
    // data directly. Roles are admin | owner | dev.
    if (
      me &&
      me.role === 'dev' &&
      me.allowedServices &&
      me.allowedServices.length > 0
    ) {
      services = services.filter((s: string) => me.allowedServices?.includes(s));
    }
    return services;
  }, [serviceNameValues, me]);
