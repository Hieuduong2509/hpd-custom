...
export const buildOtelCollectorConfig = (
  ...
  // Additional named per-team API keys (from the TeamApiKey collection), on
  // top of each team's original single `apiKey`. Bearer-token auth here only
  // validates "is this any team's key" — it doesn't attribute ingested data
  // to a particular key or team (see hyperdx-rbac-implementation-plan.md) —
  // so every active key, from either source, is just another valid token in
  // the same flat allow-list.
  extraApiKeys: string[] = [],
): CollectorConfig => {
  const apiKeys = teams
    .filter(team => team.apiKey)
    .map(team => team.apiKey)
    .concat(extraApiKeys);
      ...
      async span => {
        try {
          ...
          if (acceptsRemoteConfig) {
            ...
            const extraApiKeys = (
              await TeamApiKey.find({ revokedAt: null }, ['key'])
            ).map(k => k.key);
            const otelCollectorConfig = buildOtelCollectorConfig(
              teams,
              extraApiKeys,
            );
