...
const proxyMiddleware: RequestHandler =
  ...
  createProxyMiddleware({
    ...
    pathRewrite: function (path, req) {
      ...
      // Propagate team and RBAC settings for row-level security.
      // The row policies (docker/otel-collector/schema/seed/00009_tenant_
      // isolation_policies.sql) treat an ABSENT/empty custom_hdx_api_key as
      // "no filter" — that's the correct behavior for admin (sees
      // everything), so admin gets nothing added here. For owner/dev, send
      // the CSV of TeamApiKey values they're a member of; a restricted user
      // with zero assigned keys must NOT fall back to the empty-string
      // "no filter" case (that would show them everything instead of
      // nothing), so an unmatchable sentinel is sent instead.
      const role = req.user?.role;
      if (role && role !== 'admin') {
        const allowedKeys = req._hdx_allowed_api_keys ?? [];
        searchParams.set(
          'custom_hdx_api_key',
          allowedKeys.length > 0 ? allowedKeys.join(',') : '__no_access__',
        );
      }
