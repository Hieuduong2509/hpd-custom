...
import { addApiKeyMember, createTeam, isTeamExisting } from '@/controllers/team';
  ...
  async (req, res, next) => {
    try {
      ...
      (User as any).register(
        // This route only ever succeeds once per instance — isTeamExisting()
        // above gates it shut as soon as any team exists. So whoever
        // completes it is, by definition, standing up the whole instance:
        // they need 'admin' immediately (invite others, create API keys,
        // create additional teams), not the 'dev' schema default.
        new User({ email, role: 'admin' }),
        password,
  ...
  try {
    ...
    (User as any).register(
      new User({
        ...
        // teamInvite.role is 'owner' | 'dev' (schema default 'dev'), so this
        // also covers invites created before the role field existed.
      ...
      async (err: Error, user: any) => {
        ...
        // Scope the new user to whichever service/key they were invited
        // under (see POST /team/invitation) — has to happen here, after the
        // User doc exists, since TeamApiKey.members references a real user.
        if (teamInvite.apiKeyId) {
          await addApiKeyMember(teamInvite.apiKeyId, user._id);
        }
